...
app.get("/profile", requireAuth, async (req, res) => {
    try {
        ...
        <form action="/profile/change-password" method="POST">
          <input type="password" name="oldPassword" placeholder="Поточний пароль" required><br><br>
          <input type="password" name="newPassword" placeholder="Новий пароль" required><br><br>
...
// ==========================================
// ОБРОБКА ЗМІНИ ПАРОЛЯ (Частина 1 та 2)
// ==========================================
...
app.post("/profile/change-password", requireAuth, async (req, res) => {
    ...
    // ВАЛІДАЦІЯ (Частина 2): Мінімум 8 символів, 1 велика літера, 1 цифра
    const passwordRegex = /^(?=.*[A-Z])(?=.*\d).{8,}$/;
    if (!passwordRegex.test(newPassword)) {
        return res.send(`
            <div style="font-family: Arial, sans-serif; margin: 20px;">
                <h3 style="color: red;">Помилка безпеки</h3>
                <p>Новий пароль занадто слабкий! Він має містити щонайменше 8 символів, одну велику літеру та одну цифру.</p>
                <a href="/profile">Повернутися в профіль</a>
            </div>
        `);
    }

    try {
        // 1. Беремо старий пароль користувача з бази
        ...
        if (!isMatch) {
            return res.send(`
                <div style="font-family: Arial, sans-serif; margin: 20px;">
                    ...
                    <p>Поточний пароль вказано невірно!</p>
                    <a href="/profile">Спробувати знову</a>
                </div>
            `);
        }

        // 3. Хешуємо новий пароль та оновлюємо базу
        ...
        res.send(`
            <div style="font-family: Arial, sans-serif; margin: 20px;">
                <h3 style="color: green;">Успіх!</h3>
                <p>Пароль було успішно оновлено.</p>
                <a href="/profile">Повернутися в профіль</a>
            </div>
        `);
    } catch (err) {
        console.error(err);
        res.status(500).send("Помилка сервера під час зміни пароля");
    }
});
app.listen(PORT, () => {
